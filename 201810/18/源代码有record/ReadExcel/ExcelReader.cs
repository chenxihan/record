...
namespace DLLComplierEditor
{
    ...
    class ExcelReader
    {
        ...
        public void LoadExcelFromFolder(string path, string declareDir, string dataDir)
        {
            ...
            try
            {
                ...
                */
                ...
                string declaresPath = "Assets/Plugins/References/FuncellDeclare";
                if (Directory.Exists(declaresPath))
                    Directory.Delete(declaresPath, true);
                Directory.CreateDirectory(declaresPath);
                string luaDataPath = "Assets/StreamingAssets/Lua/Config";
                if (Directory.Exists(luaDataPath))
                    Directory.Delete(luaDataPath, true);
                Directory.CreateDirectory(luaDataPath);

                string[] Files = Directory.GetFiles(declareDir);
                int count = 0;
                foreach (var item in Files)
                {
                    string[] str = item.Replace("\\", "/").Split('/');
                    new Thread(new ThreadStart(() => CopyFiles(item.Replace("\\", "/"), declaresPath + "/" + str[str.Length - 1], ref count))).Start();
                }

                // 让主线程等待导表执行完成然后生成工程
                while (count >= Files.Length)
                {
                    UnityEditor.EditorUtility.DisplayProgressBar("配置信息", string.Format("请稍等,正在努力copy文件... ...{0}/{1}", count, Files.Length), (float)count / (float)Files.Length);
                    Thread.Sleep(1000);
                }

                UnityEditor.EditorUtility.ClearProgressBar();
                string[] Files2 = Directory.GetFiles(Defines.CN_CONFIG_DATA_CODE_SOURCE_PATH_LUA);
                int count2 = 0;
                foreach (var item in Files2)
                {
                    string[] str = item.Replace("\\", "/").Split('/');
                    new Thread(new ThreadStart(() => CopyFiles(item.Replace("\\", "/"), luaDataPath +"/"+ str[str.Length - 1], ref count2))).Start();
                }

                // 让主线程等待导表执行完成然后生成工程
                while (count2 >= Files2.Length)
                {
                    UnityEditor.EditorUtility.DisplayProgressBar("配置信息", string.Format("请稍等,正在努力copy文件2... ...{0}/{1}", count2, Files2.Length), (float)count2 / (float)Files2.Length);
                    Thread.Sleep(1000);
                }
            }
            catch (Exception ex)
        ...
        private static void CopyFiles(string originalPath, string targetPath,ref int count)
        {
