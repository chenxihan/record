//文件是自动生成,请勿手动修改.来自数据文件:achMedal
using System;
using System.Collections.Generic;
using XLua;
namespace Funcell.Cfg.Data
{
  [global::System.Serializable, global::ProtoBuf.ProtoContract(Name=@"DeclareAchMedal")]
  public class DeclareAchMedal
  {
    #region //私有变量定义
    //属性值（属性类型id_基础值）(@;@_@)
    private string _att;
    ...
    #region //Create函数的定义
    public static DeclareAchMedal Create( string in_att, int in_icon, int in_level, string in_name, int in_radio, int in_upNeedAch )
    {
      DeclareAchMedal tmp = new DeclareAchMedal();
      tmp._att = in_att;
      tmp._icon = in_icon;
      tmp._level = in_level;
      tmp._name = in_name;
    ...
    //填充数据回调
    public delegate void OnFillDataHandler(Dictionary<int, DeclareAchMedal> data);
    private static OnFillDataHandler _fillDataCallBack = null;
    //数据存储字典
    private static Dictionary<int, DeclareAchMedal> _dataCaches = null;
    public static Dictionary<int, DeclareAchMedal> CacheData
    {
        get 
        {
            lock (_lockObj)
    ...
    public static void SetData()
         {
             if(_dataCaches == null && LuaEnv.isLoadLuaCfg)
             {
                 ...
                 Dictionary<int, int[]> dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>("DataAchMedal");
                 if (dic == null)
                  {
                       LuaEnv.SharedLuaEnv.DoString("DataAchMedal = require 'Lua/Config/DataAchMedal'");
                       dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>("DataAchMedal");                  }

                 if (dic != null)
                 {
                              _dataCaches = new Dictionary<int, DeclareAchMedal>();
                 ...
                 Record(startTime, DateTime.Now.Ticks,"AchMedal");
             }
    ...
    //根据ID获取数据
    public static DeclareAchMedal Get(int id)
    {
        lock (_lockObj)
        {
            SetData();
    ...
    //transForm函数，作数据填充
    public static void Transform(Dictionary<int, DeclareAchMedal> data)
    {
        lock (_lockObj)
        {
            _dataCaches = data;
