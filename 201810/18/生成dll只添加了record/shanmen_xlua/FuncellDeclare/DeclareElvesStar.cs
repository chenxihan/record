...
{
  [global::System.Serializable, global::ProtoBuf.ProtoContract(Name=@"DeclareElvesStar")]
  public class DeclareElvesStar
  {
    #region //私有变量定义
    //属性值(@;@_@)
    ...
    //填充数据回调
    public delegate void OnFillDataHandler(Dictionary<int, DeclareElvesStar> data);
    private static OnFillDataHandler _fillDataCallBack = null;
    //数据存储字典
    private static Dictionary<int, DeclareElvesStar> _dataCaches = null;
    public static Dictionary<int, DeclareElvesStar> CacheData
    {
        get 
        {
            lock (_lockObj)
    ...
    //根据ID获取数据
    public static DeclareElvesStar Get(int id)
    {
        lock (_lockObj)
        {
            SetData();
    ...
    //transForm函数，作数据填充
    public static void Transform(Dictionary<int, DeclareElvesStar> data)
    {
        lock (_lockObj)
        {
            _dataCaches = data;
