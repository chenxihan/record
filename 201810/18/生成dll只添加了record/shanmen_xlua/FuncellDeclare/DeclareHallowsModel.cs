...
{
  [global::System.Serializable, global::ProtoBuf.ProtoContract(Name=@"DeclareHallowsModel")]
  public class DeclareHallowsModel
  {
    #region //私有变量定义
    //激活增加属性(@;@_@)
    ...
    [global::ProtoBuf.ProtoMember(7, IsRequired = true, Name=@"Res", DataFormat = global::ProtoBuf.DataFormat.Default)]
    ...
    [global::ProtoBuf.ProtoMember(10, IsRequired = true, Name=@"UiRes", DataFormat = global::ProtoBuf.DataFormat.Default)]
    public string UiRes { get{ return _ui_Res; } set{ _ui_Res=value; } }
    #endregion

    #region //Create函数的定义
    public static DeclareHallowsModel Create( string in_active_attr, string in_active_item, int in_icon, int in_id, int in_max_level, int in_notice, string in_res, string in_train_attr, string in_train_item, string in_ui_Res )
    {
      DeclareHallowsModel tmp = new DeclareHallowsModel();
    ...
    //填充数据回调
    public delegate void OnFillDataHandler(Dictionary<int, DeclareHallowsModel> data);
    private static OnFillDataHandler _fillDataCallBack = null;
    //数据存储字典
    private static Dictionary<int, DeclareHallowsModel> _dataCaches = null;
    public static Dictionary<int, DeclareHallowsModel> CacheData
    {
        get 
        {
            lock (_lockObj)
    ...
    public static void SetData()
         {
             if(_dataCaches == null && LuaEnv.isLoadLuaCfg)
             {
                 ...
                 Dictionary<int, int[]> dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>("DataHallowsModel");
                 if (dic == null)
                  {
                       LuaEnv.SharedLuaEnv.DoString("DataHallowsModel = require 'Lua/Config/DataHallowsModel'");
                       dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>("DataHallowsModel");                  }

                 if (dic != null)
                 {
                             _dataCaches = new Dictionary<int, DeclareHallowsModel>(8);
                     foreach (var item in dic)
                     {
                       ...
                       tmp.Res = CfgStringLua.Get(item.Value[6]);
                       tmp.TrainAttr = CfgStringLua.Get(item.Value[7]);
                       tmp.TrainItem = CfgStringLua.Get(item.Value[8]);
                       tmp.UiRes = CfgStringLua.Get(item.Value[9]);

                         _dataCaches.Add(item.Key, tmp);
                     }
                 }
                 Record(startTime, DateTime.Now.Ticks,"HallowsModel");
             }
    ...
    //根据ID获取数据
    public static DeclareHallowsModel Get(int id)
    {
        lock (_lockObj)
        {
            SetData();
    ...
    //transForm函数，作数据填充
    public static void Transform(Dictionary<int, DeclareHallowsModel> data)
    {
        lock (_lockObj)
        {
            _dataCaches = data;
