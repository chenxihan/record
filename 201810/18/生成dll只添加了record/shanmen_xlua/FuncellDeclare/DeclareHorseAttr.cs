...
{
  [global::System.Serializable, global::ProtoBuf.ProtoContract(Name=@"DeclareHorseAttr")]
  public class DeclareHorseAttr
  {
    #region //私有变量定义
    //本级属性(@;@_@)
    ...
    #region //Create函数的定义
    public static DeclareHorseAttr Create( string in_attribute, int in_id, string in_name, int in_nocite, int in_progress, int in_skill, string in_up_item )
    {
      DeclareHorseAttr tmp = new DeclareHorseAttr();
    ...
    //填充数据回调
    public delegate void OnFillDataHandler(Dictionary<int, DeclareHorseAttr> data);
    private static OnFillDataHandler _fillDataCallBack = null;
    //数据存储字典
    private static Dictionary<int, DeclareHorseAttr> _dataCaches = null;
    public static Dictionary<int, DeclareHorseAttr> CacheData
    {
        get 
        {
            lock (_lockObj)
    ...
    public static void SetData()
         {
             if(_dataCaches == null && LuaEnv.isLoadLuaCfg)
             {
                 ...
                 Dictionary<int, int[]> dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>("DataHorseAttr");
                 if (dic == null)
                  {
                       LuaEnv.SharedLuaEnv.DoString("DataHorseAttr = require 'Lua/Config/DataHorseAttr'");
                       dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>("DataHorseAttr");                  }

                 if (dic != null)
                 {
                             _dataCaches = new Dictionary<int, DeclareHorseAttr>(800);
                 ...
                 Record(startTime, DateTime.Now.Ticks,"HorseAttr");
             }
    ...
    //根据ID获取数据
    public static DeclareHorseAttr Get(int id)
    {
        lock (_lockObj)
        {
            SetData();
    ...
    //transForm函数，作数据填充
    public static void Transform(Dictionary<int, DeclareHorseAttr> data)
    {
        lock (_lockObj)
        {
            _dataCaches = data;
