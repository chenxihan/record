...
{
  [global::System.Serializable, global::ProtoBuf.ProtoContract(Name=@"DeclareShengwuAttr")]
  public class DeclareShengwuAttr
  {
    #region //私有变量定义
    //本级属性(@;@_@)
    ...
    #region //Create函数的定义
    public static DeclareShengwuAttr Create( string in_attribute, int in_id, string in_name, int in_nocite, int in_progress, int in_skill, string in_up_item )
    {
      DeclareShengwuAttr tmp = new DeclareShengwuAttr();
    ...
    //填充数据回调
    public delegate void OnFillDataHandler(Dictionary<int, DeclareShengwuAttr> data);
    private static OnFillDataHandler _fillDataCallBack = null;
    //数据存储字典
    private static Dictionary<int, DeclareShengwuAttr> _dataCaches = null;
    public static Dictionary<int, DeclareShengwuAttr> CacheData
    {
        get 
        {
            lock (_lockObj)
    ...
    public static void SetData()
         {
             if(_dataCaches == null && LuaEnv.isLoadLuaCfg)
             {
                 ...
                 Dictionary<int, int[]> dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>("DataShengwuAttr");
                 if (dic == null)
                  {
                       LuaEnv.SharedLuaEnv.DoString("DataShengwuAttr = require 'Lua/Config/DataShengwuAttr'");
                       dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>("DataShengwuAttr");                  }

                 if (dic != null)
                 {
                             _dataCaches = new Dictionary<int, DeclareShengwuAttr>(400);
                 ...
                 Record(startTime, DateTime.Now.Ticks,"ShengwuAttr");
             }
    ...
    //根据ID获取数据
    public static DeclareShengwuAttr Get(int id)
    {
        lock (_lockObj)
        {
            SetData();
    ...
    //transForm函数，作数据填充
    public static void Transform(Dictionary<int, DeclareShengwuAttr> data)
    {
        lock (_lockObj)
        {
            _dataCaches = data;
