...
{
  [global::System.Serializable, global::ProtoBuf.ProtoContract(Name=@"DeclareWingAttr")]
  public class DeclareWingAttr
  {
    #region //私有变量定义
    //本级属性(@;@_@)
    ...
    #region //Create函数的定义
    public static DeclareWingAttr Create( string in_attribute, int in_id, string in_name, int in_nocite, int in_progress, int in_skill, string in_up_item )
    {
      DeclareWingAttr tmp = new DeclareWingAttr();
    ...
    //填充数据回调
    public delegate void OnFillDataHandler(Dictionary<int, DeclareWingAttr> data);
    private static OnFillDataHandler _fillDataCallBack = null;
    //数据存储字典
    private static Dictionary<int, DeclareWingAttr> _dataCaches = null;
    public static Dictionary<int, DeclareWingAttr> CacheData
    {
        get 
        {
            lock (_lockObj)
    ...
    public static void SetData()
         {
             if(_dataCaches == null && LuaEnv.isLoadLuaCfg)
             {
                 ...
                 Dictionary<int, int[]> dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>("DataWingAttr");
                 if (dic == null)
                  {
                       LuaEnv.SharedLuaEnv.DoString("DataWingAttr = require 'Lua/Config/DataWingAttr'");
                       dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>("DataWingAttr");                  }

                 if (dic != null)
                 {
                             _dataCaches = new Dictionary<int, DeclareWingAttr>(800);
                 ...
                 Record(startTime, DateTime.Now.Ticks,"WingAttr");
             }
    ...
    //根据ID获取数据
    public static DeclareWingAttr Get(int id)
    {
        lock (_lockObj)
        {
            SetData();
    ...
    //transForm函数，作数据填充
    public static void Transform(Dictionary<int, DeclareWingAttr> data)
    {
        lock (_lockObj)
        {
            _dataCaches = data;
