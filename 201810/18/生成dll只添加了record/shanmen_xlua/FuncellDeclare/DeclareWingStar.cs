//文件是自动生成,请勿手动修改.来自数据文件:wing_star
using System;
using System.Collections.Generic;
using XLua;
namespace Funcell.Cfg.Data
{
  [global::System.Serializable, global::ProtoBuf.ProtoContract(Name=@"DeclareWingStar")]
  public class DeclareWingStar
  {
    #region //私有变量定义
    //本级属性(@;@_@)
    ...
    //进阶成功是否全服公告（0不公告，1公告）
    private int _notice;
    //升星消耗物品及数量(格式：物品ID，数量：多个物品以；隔开）(@;@_@)
    ...
    //填充数据回调
    public delegate void OnFillDataHandler(Dictionary<string, DeclareWingStar> data);
    private static OnFillDataHandler _fillDataCallBack = null;
    //数据存储字典
    private static Dictionary<string, DeclareWingStar> _dataCaches = null;
    public static Dictionary<string, DeclareWingStar> CacheData
    {
        get 
        {
            lock (_lockObj)
    ...
    //根据ID获取数据
    public static DeclareWingStar Get(string id)
    {
        lock (_lockObj)
        {
            SetData();
    ...
    //transForm函数，作数据填充
    public static void Transform(Dictionary<string, DeclareWingStar> data)
    {
        lock (_lockObj)
        {
            _dataCaches = data;
