...
namespace DLLComplierEditor
{
    ...
    public class CSharpWriter
    {
        ...
        public static void WriteCSharpData(string declareDir, string dataDir)
        {
            ...
            //1、加载所有lua配置数据的类
            string cfgfile2 = Defines.CN_CONFIG_PATH_LUA + "/LuaDataConverter.lua";
            cfgfile2 = cfgfile2.Replace('\\', '/');
            WriteFile(cfgfile2, GetConvertString2());

            //2、Lua数据转换文件
            string cfgfile3 = Defines.CN_CONFIG_STRING_DATA_PATH + "/LuaDataConverter.cs";
            cfgfile3 = cfgfile3.Replace('\\', '/');
            WriteFile(cfgfile3, GetConvertString3());
            ...
            //3、所有配置中的所有文字lua表
            string definefile2 = Defines.CN_CONFIG_PATH_LUA + "/StringDefines.lua";
            definefile2 = definefile2.Replace('\\', '/');
            WriteFile(definefile2, GetStringDefinesString2());

            //4、用于c#的Declare类,根据id获取lua文字表中的值
            string definefile3 = Defines.CN_CONFIG_STRING_DATA_PATH + "/CfgStringLua.cs";
        ...
        public static void WriteDataDeclareCSEx(string declareDir, string dataDir, DataTable dt, List<ColumnInfo> fieldTypeRawAry)
        {
            try
            {
                ...
                //5、创建每个配置lua表
                string file2 = Defines.CN_CONFIG_PATH_LUA + "/Data" + formatedTableUpName + ".lua";
                file2 = file2.Replace('\\', '/');
                WriteFile(file2, string.Equals(formatedTableUpName, "messagestring", StringComparison.OrdinalIgnoreCase) ? GetMessageStringImplementClass2(dt, formatedTableUpName) : GetImplementString2(dt, formatedTableUpName, fieldTypeRawAry));
        ...
        private static void WriteFile(string fileName, string cSharpString)
        {
            try
            {
                ...
                {
                    Directory.CreateDirectory(Defines.CN_CONFIG_PATH_LUA);
                }
        ...
        private static string GetConvertString2()
        {
            ...
            foreach (DataTable dt in _tableList)
            {
                sb.AppendFormat("Data{0} = require('Lua/Config/Data{1}')", FormartName(dt.TableName), FormartName(dt.TableName));
                sb.AppendLine();
            }
            sb.AppendLine("function RemoveAllCfg()");

            foreach (DataTable dt in _tableList)
            {
                sb.AppendFormat("   Data{0} = nil", FormartName(dt.TableName));
                sb.AppendLine();
                sb.AppendFormat("   RemoveRequiredByName(\"Lua/Config/Data{0}\")", FormartName(dt.TableName));
                sb.AppendLine();
            }
            sb.AppendLine("end");
            return sb.ToString();
        ...
        //转换类的实现 LuaDataConverter
        private static string GetConvertString3()
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("//文件是自动生成,请勿手动修改.此类只有一个接口用于填充字典数据.");
            sb.AppendLine("using Funcell.Cfg.Data;");
            sb.AppendLine("using System.Collections.Generic;");
            sb.AppendLine("namespace XLua");
            sb.AppendLine("{");
            sb.AppendLine("  public class LuaDataConverter");
            sb.AppendLine("  {");
            sb.AppendLine("     private static bool _isLoaded = false;");
            sb.AppendLine("     public static bool IsLoaded()");
            sb.AppendLine("     {");
            ...
            {
                tableName = FormartName(dt.TableName);
                sb.AppendLine("       Declare" + tableName + ".SetLuaDataCallBack(GetLuaData);");
                sb.AppendLine("       Declare" + tableName + ".SetStringCallBack(GetString);");
            }
            sb.AppendLine("        _isLoaded = true;");
            sb.AppendLine("     }");

            sb.AppendLine("     private static Dictionary<int, int[]> GetLuaData(string name)");
            sb.AppendLine("     {");
            sb.AppendLine("         Dictionary<int, int[]> dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>(name);");
            sb.AppendLine("         if (dic == null)");
            sb.AppendLine("         {");
            sb.AppendLine("             LuaEnv.SharedLuaEnv.DoString(string.Format(\"{0} = require 'Lua/Config/{1}'\",name,name));");
            sb.AppendLine("             dic = LuaEnv.SharedLuaEnv.Global.Get<Dictionary<int, int[]>>(name);");
            sb.AppendLine("         }");
            sb.AppendLine("         return dic;");
            sb.AppendLine("     }");

            sb.AppendLine("     private static string GetString(int id)");
            sb.AppendLine("     {");
            sb.AppendLine("         return CfgStringLua.Get(id);");
            sb.AppendLine("     }");
        ...
        private static string GetStringDefinesString3()
        {
            ...
            sb.AppendLine("                     strArr = LuaEnv.SharedLuaEnv.Global.Get<string[]>(\"StringDefines\");");
            sb.AppendLine("                 }");
            sb.AppendLine("                 Record(startTime, DateTime.Now.Ticks,\"Lua_StringDefines\");");
            sb.AppendLine("             }");
            ...
            sb.AppendLine("    {");
            sb.AppendLine("         System.IO.FileStream fs = new System.IO.FileStream(\"E:/NewConfig.txt\", System.IO.FileMode.OpenOrCreate, System.IO.FileAccess.ReadWrite);");
            sb.AppendLine("         int r = fs.Read(new byte[102400], 0, 102400);");
        ...
        private static string GetDeclareString(DataTable dt, string TableUpName, List<ColumnInfo> fieldTypeRawAry)
        {
            ...
            sb.AppendLine("    //从lua端填充数据回调");
            sb.AppendLine("    public delegate Dictionary<int, int[]> OnLuaDataHandler(string name);");
            sb.AppendLine("    private static OnLuaDataHandler _luaDataCallBack = null;");
            sb.AppendLine("    //获取文字回调");
            sb.AppendLine("    public delegate string OnSetStringHandler(int id);");
            sb.AppendLine("    private static OnSetStringHandler _setStingCallBack = null;");
            ...
            sb.AppendLine("         {");
            sb.AppendLine("             if(_dataCaches == null && _luaDataCallBack !=null && _setStingCallBack != null)");
            sb.AppendLine("             {");
            sb.AppendLine("                 long startTime = DateTime.Now.Ticks;");
            sb.AppendFormat("                 Dictionary<int, int[]> dic = _luaDataCallBack(\"Data{0}\");", TableUpName);
            ...
            for (int i = 0; i < fieldTypeAry.Count; i++)
            {
                ...
                if (GenFieldType(fInfo) == "string")
                    sb.AppendFormat("                       tmp.{0} = _setStingCallBack(item.Value[{1}]);", FormartName(fInfo.m_name), num);
                else
            ...
            if (KeyType == "string")
            {
                sb.AppendLine("                         _dataCaches.Add(_setStingCallBack(item.Key), tmp);");
            }
            else
            {
            ...
            sb.AppendLine("                     }");
            sb.AppendLine("                 }");
            sb.AppendLine("                 Record(startTime, DateTime.Now.Ticks,\"Lua_" + TableUpName + "\");");
            sb.AppendLine("             }");
            ...
            sb.AppendLine("    //设置从lua端填充数据回调");
            sb.AppendLine("    public static void SetLuaDataCallBack(OnLuaDataHandler callBack)");
            sb.AppendLine("    {");
            sb.AppendLine("        _luaDataCallBack = callBack;");
            sb.AppendLine("    }");

            sb.AppendLine("    //设置获取文字回调");
            sb.AppendLine("    public static void SetStringCallBack(OnSetStringHandler callBack)");
            sb.AppendLine("    {");
            sb.AppendLine("        _setStingCallBack = callBack;");
            sb.AppendLine("    }");
        ...
        private static string GetMessageStringDeclareClass(DataTable dt, string TableUpName)
        {
            ...
            sb.AppendLine("                 foreach (var item in dic)");
            sb.AppendLine("                 {");
            sb.AppendLine("                     _dataCaches.Add(item.Key, _setStingCallBack(item.Value[0]));");
            sb.AppendLine("                  }");
            sb.AppendLine("              }");
            sb.AppendLine("              LoadRecord.Record(startTime, DateTime.Now.Ticks, \"Lua_MessageString\");");
            sb.AppendLine("         }");
            sb.AppendLine("         if (_dataCaches == null && _fillDataCallBack != null)");
            sb.AppendLine("         {");
            sb.AppendLine("             if (_dataCaches == null)");
            ...
            sb.AppendLine("                 _dataCaches = new Dictionary<int, string>();");
            sb.AppendLine("                 _fillDataCallBack(_dataCaches); ");
            sb.AppendLine("                 LoadRecord.Record(startTime, DateTime.Now.Ticks, \"MessageString\");");
            sb.AppendLine("              }");
            sb.AppendLine("         }");
            sb.AppendLine("    }");
            ...
            sb.AppendLine("  }");
            //Cllss
            sb.AppendLine("  public class LoadRecord");
            sb.AppendLine("  {");
            //Record
            sb.AppendLine("    public static void Record(long startTime,long stopTime,string name)");
            sb.AppendLine("    {");
            sb.AppendLine("         System.IO.FileStream fs = new System.IO.FileStream(\"E:/NewConfig.txt\", System.IO.FileMode.OpenOrCreate, System.IO.FileAccess.ReadWrite);");
            sb.AppendLine("         int r = fs.Read(new byte[102400], 0, 102400);");
            sb.AppendLine("         if (r > 0)");
        ...
        private static string GetMessageStringImplementClass2(DataTable dt, string TableUpName)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("--文件是自动生成,请勿手动修改.来自数据文件:" + dt.TableName);
            sb.AppendLine("return {");
            for (int r = 0; r < dt.Rows.Count; ++r)
            {
                ...
                var intKey = row[1];
                var str = row[2];
                sb.AppendFormat("   [{0}] = {1},", Convert.ToInt32(intKey), ProcessStringDefines(str.ToString(), TableUpName, true));
                sb.AppendLine();
            }
            sb.AppendLine("}");
            return sb.ToString();
