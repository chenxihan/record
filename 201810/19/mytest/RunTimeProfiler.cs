...
public class RuntimeProfiler : MonoBehaviour
{
    ...
    private void ExpansionGUIWindow(int windowId)
    {
        ...
        GUI.contentColor = (_debugType == DebugType.myTest ? Color.white : Color.gray);
        if (GUILayout.Button("myTest", GUILayout.Height(50)))
        {
            GameObject.FindObjectOfType<Test_For>().GetComponent<Test_For>().StartTest();
        }
        ...
        else if (_debugType == DebugType.Memory)
        {
            ...
            GUILayout.Label("总内存：" + ((float)Profiler.GetTotalAllocatedMemory())/ 1000000 + "MB");
            GUILayout.Label("已占用内存：" + ((float)Profiler.GetTotalAllocatedMemory()) / 1000000 + "MB");
            GUILayout.Label("空闲中内存：" + ((float)Profiler.GetTotalUnusedReservedMemory()) / 1000000 + "MB");
            GUILayout.Label("总Mono堆内存：" + ((float)Profiler.GetMonoHeapSize()) / 1000000 + "MB");
            GUILayout.Label("已占用Mono堆内存：" + ((float)Profiler.GetMonoUsedSize()) / 1000000 + "MB");
#endif
...
#if UNITY_5
            GUILayout.Label("项目ID：" + Application.cloudProjectId);
#endif
#if UNITY_7
